...
//				});
	...
	public Quiz newQuiz2(final Quiz quiz){
		final String sql = "INSERT INTO `" + Setting.DBNAME
			+ "`.`ent_jquiz` (`AuthorID`, `GroupID`, `Title`, `Description`, `Code`, "
			+ "`MinVar`, `MaxVar`, `AnsType`, `Privacy`, `rdfID`, `QuesType`, `timestamp`, "
			+ "`rdfIdDisplay`, `version`,`start`,`end`) "
			+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)";
		...
		getJdbcTemplate().update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				...
				pStatement.setString(13, Quiz.getStrWithPrefix(quiz.getRdfId().toString()).toString());
				pStatement.setInt(14, quiz.getVersion());
				pStatement.setInt(15, quiz.getLine_start());
				pStatement.setInt(16, quiz.getLine_end());
				return pStatement;
			}
		}, keyHolder);
	...
	public Integer updateQuiz(Quiz quiz){
		...
		final String sql = "UPDATE `webex21`.`ent_jquiz` SET `Title` = ?, `Description` = ?, `Privacy` = ?, `timestamp` = ?, `start`=?, `end`=? WHERE `QuizID` = ?";
		return getJdbcTemplate().update(
				sql,
				new Object[]{
