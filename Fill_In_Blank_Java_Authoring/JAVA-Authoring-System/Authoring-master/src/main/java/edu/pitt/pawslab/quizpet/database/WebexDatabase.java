...
public class WebexDatabase extends JdbcDaoSupport {
	...
	public Quiz newQuiz2(final Quiz quiz){
		final String sql = "INSERT INTO `" + Setting.DBNAME
			+ "`.`ent_jquiz` (`AuthorID`, `GroupID`, `Title`, `Description`, `Code`, `MinVar`, `MaxVar`, `AnsType`, `Privacy`, `rdfID`, `QuesType`,`start`,`end`) "
			+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)";
		...
		getJdbcTemplate().update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				...
				pStatement.setString(10, quiz.getRdfIdInDb());
				pStatement.setInt(11, quiz.getQuestionTypeId());
				pStatement.setInt(12, quiz.getLine_start());
				pStatement.setInt(13, quiz.getLine_end());
				return pStatement;
			}
		}, keyHolder);
	...
	public Integer updateQuiz(Quiz quiz){
		...
		final String sql = "UPDATE `" + Setting.DBNAME + "`.`ent_jquiz` SET `Title` = ?, `Description` = ?, `Privacy` = ?,`start` = ?,`end` = ? WHERE `QuizID` = ?";
		return getJdbcTemplate().update(
				sql,
				new Object[]{
	...
	public HashMap<Integer, StringBuilder> getAllPyClasses(){
		final String sql = "SELECT * FROM `" + Setting.DBNAME + "`.`ent_class`  where ClassName like  '%.java'";
		final HashMap<Integer, StringBuilder> result = new HashMap<Integer, StringBuilder>();
		getJdbcTemplate().query(
				sql,
				new RowMapper<Void>(){
...
//				+ "WHERE Title LIKE ?";
		...
		if(keyword.toString().equals("-1")){
			final String sql = "SELECT `a`.`QuizID`, `a`.`Title`, `a`.`rdfID` , `a`.`AuthorId`, `d`.`name` , `a`.`Description`"
					...
					+ "WHERE  d.id = a.AuthorId and a.QuizID = b.QuizID and  c.QuestionID = b.QuestionID and c.domain ='java' and (a.AuthorId = ? or a.Privacy = '1')";
