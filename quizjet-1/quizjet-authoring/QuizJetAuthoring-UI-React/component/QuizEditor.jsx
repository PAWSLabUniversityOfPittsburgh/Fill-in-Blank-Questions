...
var ItemList=React.createClass({
	render:function(){
		return(
			<option>{this.props.items}</option>
		)
	}
});
var ListCovering=React.createClass({
	chooseHandle:function(ev){	
		this.props.changeLineCovering(parseInt(ev.target.value),this.props.type)
	},
	render:function(){
		return (
			<select className="form-control" onChange={this.chooseHandle}>
				{
					this.props.arrList.map(function(item,index){
						 return <ItemList key={index} items={item}/>					
						})
				}
			</select>
		)
...
module.exports = React.createClass({
	...
	changeLineCovering:function(value,type){
		var quizState = this.state.quiz;
		if(type==='start'){
			quizState['line_start']=value;
			quizState['line_end']=value;
		}else{
			quizState['line_end']=value;
		}
		quizState['start_array'] = this.getArrayList(1,quizState['tot_lines']);
		quizState['end_array'] = this.getArrayList(quizState['line_start'],quizState['tot_lines']);
		console.log(quizState['line_start']);
		console.log(quizState['line_end']);
		this.setState({"quiz": quizState});
	},
	getArrayList:function(start,end){
		var array=[];
	...
	handleCodeChange: function(value){
		...
		quizState['tot_lines']=counter;
		quizState['start_array'] = this.getArrayList(1,counter);
		
		quizState['line_start']=1;

		quizState['end_array'] = this.getArrayList(quizState['line_start'],counter);
		//******* modify end*******
  ...
  <div className="modal-dialog modal-lg">
    <div className="modal-content">
      ...
      <div className="modal-body">
			<p style = {fsize}> (1) The class name must be Tester, the test method is "public static void main(String[] args){'{}'}", add you test code between {'{'} and {'}'}.</p>
		<p style = {fsize}> (2) The code must have the parameter variable named '_Param', please see the following example:</p>
			<img   src="resources/images/p5.png" data-pin-nopin="true"/>
			<div><p style = {fsize}> (3) If the assesment type is final 'value', the final vaule to be assessed must be named as "result"</p></div>
			<img   src="resources/images/p6.png" data-pin-nopin="true"/> 
      </div>
...
<button type="button" className="btn btn-default" data-dismiss="modal">Cancel</button>
				...
				<form className="form-horizontal">
					...
					{/*The lines which needs covering*/}
					<div className="form-group">
						<label className="col-sm-2 control-label">Covering lines from</label>
						<div className="col-sm-2">
							<ListCovering arrList={this.state.quiz.start_array} type="start" changeLineCovering={this.changeLineCovering}/>
						</div>
						<label className="col-sm-2 control-label">to</label>
						<div className="col-sm-2">
							<ListCovering arrList={this.state.quiz.end_array} type="end" changeLineCovering={this.changeLineCovering}/>
						</div>
					  </div>
					{/* Minimum Value */}
					<div className="form-group">
